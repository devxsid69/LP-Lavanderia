...
namespace WASistEti
{
	...
	public class WAQueries
	{
		...
		//private const string sCadenaConexion = @"Provider=Microsoft.ACE.Sql.12.0; Data Source = c:\BD LP ETAPA 2\BD - LP - Lavado y Confeccion.accdb";
		public WAQueries()
		{
			//
		...
		private static SqlConnection Conection()
		{
			...
			SqlConnection odcConexion = new SqlConnection(obtenCadenaDeConexionTPS);
			odcConexion.Open(); 
			return odcConexion;
		}
...
WHERE tbCuttingTicketDet.ID ="+idEtiqueta+@"";
		...
		public static DataSet LlenaListaCT(string sCorte)
		{
			...
			if(sCorte!="")
			{
				sQuery = @"Select id, NoCorte,Corte, Contrato, Estilo, Color From vtCuttingTicket where Corte = """+sCorte+@""" order by id desc";
			}
			else
			{
				sQuery = @"Select id, NoCorte,Corte, Contrato, Estilo, Color From vtCuttingTicket order by id desc";
			}
			//SqlConnection odcConexion = Conection();
			//SqlCommand odcSelect = new SqlCommand(sQuery,odcConexion);
			//SqlDataAdapter odaAdapter = new SqlDataAdapter(odcSelect);
...
WHERE tbCuttingTicket.NoCorte= """+slCorte+@"""";
		...
		#endregion LlenaListaEtiqPorCorte

		public static DataSet llenaDetalleEtiCorte(string slCorte)
		{
			DataSet dsInf = new DataSet();
			string slQuery = @" select Corte,Etiqueta,UPC,COLOR,TALLA,PRENDAS,[tipo caja],case Est when 1 then 'true' else 'false' end as Estado,Existe from vtTicketInformacion Where Corte='"+slCorte+@"' order by Etiqueta asc; ";
			dsInf = LlenaDataSet(slQuery);
			return(dsInf);

		}

		public static DataSet llenaDetalleEtiCorteId(string slCorteId)
		{
			DataSet dsInf = new DataSet();
			string slQuery = @" select NoExistRep from tbCuttingTicket Where Id="+slCorteId+" ";
			dsInf = LlenaDataSet(slQuery);
			return(dsInf);

		}

		public static int ActualizaInfoSTEXEtiqueta(string slEtiqueta,string slEstado,string slExiste)
		{
			int iResult = 0;			
			
			string sQuery = @"Update tbTicketDet set Estado = "+slEstado+@", existe = "+slExiste+@" where EtiquetaId ="+slEtiqueta+"";
			try
			{
			SqlConnection odcConexion = Conection();
			SqlCommand odcInsert = new SqlCommand(sQuery,odcConexion);
		...
		public static int InicializaInfoSTEXEtiqueta(string slCorte)
		{

			int iResult = 0;			
			
			string sQuery = @"Update vtTicketInformacion set Est = 0, existe = 0 where corte ="""+slCorte+@"""";
			try
			{
				SqlConnection odcConexion = Conection();
		...
		#region public static int ActualizaCorte
		public static int ActualizaCorte(string slIdCorte,string slNoExisteRep,string slUsuarioRev)
		{
			int iResult = 0;			
			
			string sQuery = @"
update tbCuttingTicket
			Set
				NoExistRep="""+slNoExisteRep+@""",
				UsuarioRev="""+slUsuarioRev+@"""	where id="+slIdCorte;
			iResult = EjecutaInsertRegresaRegistro(sQuery);
			return iResult;
		}
		#endregion //public static int InsertaCorte(string sCorte, string sContrato, string sEstilo, string sColor)
